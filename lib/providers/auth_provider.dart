...
//       final existingUsers = _userBox.values.where((user) => user.email == email);
//
//       // 2. If the list of existing users is NOT empty, it means the email is taken.
//       if (existingUsers.isNotEmpty) {
//         state = AuthError('User already exists');
//         // Delay to allow the UI to show the loading state before the error.
//         await Future.delayed(const Duration(milliseconds: 500));
//         _checkAuthStatus(); // Revert to previous state
//         return false;
//       }
//       // *** END OF CORRECTION ***
//
//
//       // Create new user if the email is not taken
//       final userId = DateTime.now().millisecondsSinceEpoch.toString();
//       final user = User(
//         id: userId,
//         email: email,
...
//   late final Box<bool> _onboardingBox;
//
//   // This Completer will help us wait for the initialization to finish
...
//     } catch (e) {
//       state = AuthError('Failed to initialize auth: $e');
//       // Mark initialization as complete with an error
//       if (!_initCompleter.isCompleted) {
...
//
//   Future<void> completeOnboarding() async {
//     // Wait for init to be complete before proceeding
...
//         state = AuthError('User already exists');
//         await Future.delayed(const Duration(milliseconds: 500));
//         state = Unauthenticated(); // Revert to a stable state
//         return false;
//       }
//
//       final userId = DateTime.now().millisecondsSinceEpoch.toString();
...
//       final user = _userBox.values.firstWhere(
//             (user) => user.email == email,
//         orElse: () => throw Exception('User not found'), // Throw a specific error
//       );
//
//       final updatedUser = user.copyWith(lastLoginAt: DateTime.now());
//       await _userBox.put(user.id, updatedUser);
...
//       return true;
//     } catch (e) {
//       state = AuthError('Sign in failed: User not found or wrong password');
//       await Future.delayed(const Duration(milliseconds: 500));
//       state = Unauthenticated(); // Revert to a stable state
//       return false;
//     }
...
import 'dart:async'; // <-- CORRECTED THIS LINE
import 'package:flutter_riverpod/flutter_riverpod.dart';
import 'package:hive_flutter/hive_flutter.dart';
import '../models/user.dart';
...
class AuthNotifier extends StateNotifier<AuthState> {
  ...
  Future<bool> signIn({
    ...
    try {
      ...
      state = AuthError('Sign in failed: Invalid email or password');
      await Future.delayed(const Duration(milliseconds: 500));
      state = Unauthenticated();
      return false;
    }
