...
class TodayScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // We remove the Scaffold and FAB from here, returning only the content.
    return SafeArea(
      child: SingleChildScrollView(
        // The padding is adjusted to make sure content clears the bottom nav bar.
        padding: const EdgeInsets.only(
          left: 20,
          right: 20,
          top: 20,
          bottom: 100, // Adjusted padding for the custom nav bar
        ),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
  ...
  String _getFormattedDate() {
    ...
    // In Rwanda, the week starts on Monday.
    final dayOfWeek = days[now.weekday-1];
    return '$dayOfWeek, ${months[now.month - 1]} ${now.day}';
  }
}